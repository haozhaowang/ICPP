...
parser.add_argument('--hetero-allocation', type=str, default='1.0-1.0') # deprecated
parser.add_argument('--stale-threshold', type=int, default=1000)    # only a position argument
...
def run(rank, model, train_data, test_data, queue, param_q, stop_flag):
    ...
    for epoch in range(int(args.epochs)):
        ...
        for batch_idx, (data, target) in enumerate(train_data):
            ...
            time_logs.write(str(it_duration) + "\t" +
                            str(it_comp_duration) + "\t" +
